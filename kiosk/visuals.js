...
const FRAG = /* glsl */ `
  ...
  float particleLayer(
    vec2 p, float scale, float seed,
    float density, float size, float twinkleSpeed
  ) {
    vec2 grid = p * scale;
    vec2 cell = floor(grid);
    vec2 local = fract(grid) - 0.5;
    vec2 rnd = (hash22(cell + seed) - 0.5) * 0.82;
    vec2 delta = local - rnd;
    float dist = length(delta);
    float enabled = step(1.0 - density, hash21(cell + seed * 7.31));
    float core = 1.0 - smoothstep(size * 0.22, size, dist);
    float halo = 1.0 - smoothstep(size, size * 2.45, dist);
    float twinklePhase = hash21(cell + seed * 3.17) * TAU;
    float speedRnd = mix(0.65, 1.35, hash21(cell + seed * 9.12));
    float twinkle = 0.80 + 0.20 * sin(uTime * twinkleSpeed * speedRnd + twinklePhase);
    return enabled * (core + halo * 0.10) * twinkle;
  }
  ...
  vec3 renderPulse(
    vec2 uv, vec2 dir, vec2 tangent,
    float sphereDist, float phase, float strength,
    float seed, float audioEnergy, float orbitAngle
  ) {
    float p = fract(phase);
    float t = easeOut(p);

    float appear = smoothstep(0.00, 0.06, p);
    float fade = 1.0 - smoothstep(0.60, 1.00, p);

    float ignitionWidth = mix(0.007, 0.016, smoothstep(0.0, 0.28, p));
    float ignition = exp(-pow((sphereDist - 0.004) / ignitionWidth, 2.0));
    ignition *= 1.0 - smoothstep(0.12, 0.38, p);

    float shellCenter = mix(0.006, 0.150, t);
    float shellWidth  = mix(0.010, 0.037, t);
    float shell = exp(-pow((sphereDist - shellCenter) / shellWidth, 2.0));

    float sideBias = pow(abs(dir.x), 1.25);
    float sideTransition = smoothstep(0.18, 0.72, p);
    float angularShape = mix(1.0, 0.34 + sideBias * 1.40, sideTransition);

    float breakup = 0.55 + 0.45 * smoothstep(
      -0.45, 0.60,
      snoise(dir * (2.1 + seed * 0.2) + vec2(uTime * 0.034, seed * 4.7))
    );

    float envelope = (ignition * 0.95 + shell * angularShape);
    envelope *= appear * fade * strength * breakup;

    float innerMask = smoothstep(-0.007, 0.001, sphereDist);
    float outerMask = 1.0 - smoothstep(0.165, 0.230, sphereDist);
    envelope *= innerMask * outerMask;

    vec2 curl = curlNoise2D(uv * 4.6 + vec2(seed * 7.3, uTime * 0.040));

    float radialTravel = t * (0.022 + strength * 0.010);

    // ---- tangential swirl along the rim, boosted by audio (движение по краю) ----
    float baseSwirl = sin(uTime * 0.34 + length(uv) * 15.0 + seed * 2.7)
      * (0.0012 + t * 0.0048);
    float audioSwirl = sin(uTime * 1.15 + seed * 3.1 + length(uv) * 9.0)
      * audioEnergy * (0.0030 + t * 0.0070);
    float swirl = baseSwirl + audioSwirl;

    // slow continuous orbital drift so the halo always "breathes" around the edge
    float orbit = (0.0016 + audioEnergy * 0.0060) * t;

    vec2 particleSpace =
      uv
      - dir * radialTravel
      + tangent * (swirl + orbit)
      + curl * (0.0028 + t * 0.0058);

    // orbit the whole sparkle pattern around the centre (visible even in silence)
    particleSpace = rotate2D(orbitAngle * (0.6 + t * 0.6)) * particleSpace;

    vec2 pA = particleSpace;
    vec2 pB = rotate2D(0.42 + seed * 0.03) * (particleSpace + curl * 0.0017);
    vec2 pC = rotate2D(-0.58 - seed * 0.02) * (particleSpace - curl * 0.0012);
    vec2 pD = rotate2D(0.93 + seed * 0.01) * (particleSpace + tangent * 0.0010);

    float layerA = particleLayer(pA, 520.0, seed + 1.1, 0.44, 0.145, 0.72);
    float layerB = particleLayer(pB, 820.0, seed + 7.2, 0.30, 0.124, 0.90);
    float layerC = particleLayer(pC, 1180.0, seed + 13.7, 0.20, 0.108, 1.06);
    float layerD = particleLayer(pD, 860.0, seed + 21.4, 0.060, 0.114, 0.66);

    float leadingEdge = exp(
      -pow((sphereDist - shellCenter) / max(shellWidth * 0.55, 0.006), 2.0)
    );

    float powderA = fbm(particleSpace * 34.0 + vec2(uTime * 0.060, -uTime * 0.045));
    float powderB = snoise(particleSpace * 82.0 + vec2(-uTime * 0.085, uTime * 0.065));
    float powder = smoothstep(0.16, 0.68, powderA * 0.76 + powderB * 0.24);

    float purpleIntensity = (layerA * 0.85 + layerB * 0.68 + layerC * 0.46) * envelope;
    float whiteIntensity  = layerD * envelope * (0.32 + leadingEdge * 0.92);
    float powderIntensity = powder * envelope * 0.085;

    vec3 color = vec3(0.0);
    color += particleColors(0.00, 0.55, 0.15, purpleIntensity * (0.88 + audioEnergy * 0.92));
    color += particleColors(0.10, 0.70, 0.10, layerB * envelope * 0.16);
    color += particleColors(0.90, 0.25, 0.00, whiteIntensity * (0.60 + audioEnergy * 0.58));
    color += particleColors(0.10, 0.45, 0.05, powderIntensity);
    return color;
  }
  ...
  vec3 renderIntroRing(vec2 uv, vec2 dir, vec2 tangent, float radius, float reveal) {
    ...
    // swirling sparks that ride the expanding shell
    vec2 curl = curlNoise2D(uv * 5.0 + vec2(3.1, uTime * 0.05));
    vec2 pspace =
      uv
      - dir * t * 0.020
      + tangent * sin(uTime * 0.6 + radius * 20.0) * 0.0022
      + curl * (0.0030 + t * 0.0040);

    float la = particleLayer(pspace, 560.0, 3.3, 0.50, 0.140, 0.80);
    float lb = particleLayer(rotate2D(0.5) * pspace, 900.0, 9.1, 0.34, 0.120, 1.00);
    float lw = particleLayer(rotate2D(-0.4) * pspace, 820.0, 15.2, 0.10, 0.110, 0.70);

    vec3 c = vec3(0.0);
    c += particleColors(0.00, 0.60, 0.15, (la * 0.90 + lb * 0.60) * env * 1.35);
    c += particleColors(0.92, 0.20, 0.00, lw * env * (0.5 + leading * 1.0) * 1.20);
  ...
  void main() {
    ...
    // =======================================================
    // STEADY DUST AROUND THE SPHERE
    // =======================================================
    vec2 steadyCurl = curlNoise2D(uv * 4.0 + vec2(uTime * 0.028, -uTime * 0.023));

    // continuous orbital rotation of the dust ring; sound speeds it up
    float orbitAngle = uTime * 0.12 + uParticleTime * 0.05 + audioEnergy * uTime * 0.55;

    // audio-reactive tangential flow so dust drifts along the edge on sound
    float edgeFlow = sin(uTime * 0.28 + radius * 14.0) * (0.0026 + audioEnergy * 0.0050)
      + audioEnergy * 0.0034;

    vec2 steadySpace =
      uv
      - dir * uParticleTime * 0.0012
      + steadyCurl * 0.0034
      + tangent * edgeFlow;

    // spin the sparkle pattern around the rim
    steadySpace = rotate2D(orbitAngle * 0.7) * steadySpace;

    float steadyRim   = exp(-pow((particleDistance - 0.007) / 0.013, 2.0));
    float steadyCloud = exp(-pow((particleDistance - 0.024) / 0.032, 2.0));

    float steadyMask =
      smoothstep(-0.007, 0.001, particleDistance)
      * (1.0 - smoothstep(0.105, 0.165, particleDistance));

    float steadyEnvelope = (steadyRim * 0.15 + steadyCloud * 0.040)
      * steadyMask
      * (0.55 + uActivity * 0.42 + uParticlePulse * 0.45)
      * partReveal;

    float steadyA = particleLayer(steadySpace, 640.0, 41.2, 0.26, 0.128, 0.64);
    float steadyB = particleLayer(rotate2D(-0.48) * steadySpace, 980.0, 52.6, 0.17, 0.110, 0.82);
    float steadyC = particleLayer(rotate2D(0.74) * steadySpace, 820.0, 73.1, 0.028, 0.112, 0.58);

    finalColor += particleColors(0.00, 0.55, 0.05, steadyA * steadyEnvelope * 0.80);
    finalColor += particleColors(0.10, 0.70, 0.00, steadyB * steadyEnvelope * 0.52);
    finalColor += particleColors(0.88, 0.15, 0.00, steadyC * steadyEnvelope * 0.58);
    ...
    finalColor += renderPulse(uv, dir, tangent, particleDistance, phaseA, strengthA, 1.0, audioEnergy, orbitAngle);
    finalColor += renderPulse(uv, dir, tangent, particleDistance, phaseB, strengthB, 2.0, audioEnergy, orbitAngle);
    finalColor += renderPulse(uv, dir, tangent, particleDistance, phaseC, strengthC, 3.0, audioEnergy, orbitAngle);
