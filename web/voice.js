...
(() => {
  ...
  // завершение диалога → вернуться к заставке (ждать слова «Оливия»).
  // Ловим прямые команды (стоп/хватит/достаточно) и намёки на прощание.
  const BYE_RE = /(\bстоп\b|хватит|достаточно|заканчива|закончим|заверши|до свидани|до встречи|всего доброго|всего хорошего|прощай|выключись|отключись|больше ничего|это всё|это все|ничего больше|спасибо,? (это )?(всё|все)|пока,? оливия|можешь идти|ты свободна|свободна)/i;
  ...
  function enterPassive() {
    ...
    if (state.inputMode === "sr") {
      ...
      // Firefox/сервер: слушаем кодовое слово «Оливия» серверным STT
      // (непрерывно), плюс касание экрана как запасной способ активации.
      hudText.textContent = PASSIVE_HUD;
      setHintText('Скажите <em>«Оливия»</em><br/>или коснитесь экрана');
      armTapActivation();
      wakeLoopServer();
    }
  }
