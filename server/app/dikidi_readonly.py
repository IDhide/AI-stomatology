...
class DikidiReadOnly:
    ...
    def format_for_prompt(
        ...
        if not bookings and free_slots and all(v is None for v in free_slots.values()):
            # API отвечает ошибкой по ВСЕМ дням — расписания фактически нет.
            # Без этого жёсткого варианта модель придумывает занятость и окна
            # (13.08: при «данных нет» по всем дням заявила «18:00 занято»).
            return (
                "ДОСТУП К СИСТЕМЕ ЗАПИСИ: сейчас недоступен (техническая пауза).\n"
                "• У тебя НЕТ никаких данных о записях и свободных окнах — "
                "НИКОГДА не называй конкретное время, не говори «занято» или "
                "«свободно» и не подбирай окна.\n"
                "• Собери имя, пожелание по дню/времени и телефон; скажи, что "
                "администратор проверит расписание и перезвонит согласовать.\n"
                "• Если пациент спрашивает про свою запись — попроси подождать: "
                "администратор посмотрит запись и пригласит."
            )
        if bookings:
            lines = "\n".join(
