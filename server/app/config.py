...
class Settings(BaseSettings):
    ...
    # Замеры (qa_voice.py, киосковый тракт 19.08): «свои» разные фразы
    # 0.03–0.05, живой голос с киоска 0.07–0.16; «чужие» 0.28–0.48,
    # worst-case синтетики 0.177. Поэтому high=0.17: уверенно называем
    # имя только при явном совпадении, 0.17–0.35 — мягкий переспрос.
    voice_match_threshold: float = Field(default=0.17, alias="VOICE_MATCH_THRESHOLD")
    # Мягкий порог: при совпадении между threshold и weak Оливия НЕ
    # утверждает личность, а мягко переспрашивает («Кажется, узнала вас…»).
