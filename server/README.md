...
## DIKIDI (расписание и свободные окна)

Оливия умеет смотреть записи и предлагать ближайшее свободное время
(«в десять занято, могу предложить в двенадцать»). Только чтение —
записи создаёт живой администратор.

Пока ключа нет, работает демо-расписание (`DIKIDI_DEMO=true` в `.env`).
Как подключить боевое:

1. В веб-версии [dikidi.net/business](https://dikidi.net/business) открой
   «Настройки» → «Интеграция» → «Создать ключ» (тип доступа —
   администратор). Скопируй ключ в `DIKIDI_API_KEY`.
2. `DIKIDI_COMPANY_ID` уже заполнен (215190 — из ссылки виджета).
3. Убери `DIKIDI_DEMO` (поставь `false`) и перезапусти сервер.
4. Открой документацию API в том же разделе «Интеграция» и сверь формат
   ответа со `server/app/dikidi_readonly.py` → `_fetch_bookings()` —
   это единственная функция, которую нужно поправить под боевой формат
   (путь, имена полей). Вся остальная логика (окна, промпт, подбор
   времени) от неё не зависит.
